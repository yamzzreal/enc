...
const log = (message, error = null) => {
    ...
    const prefix = `\x1b[36m[ 𝐘𝐚𝐦𝐳𝐳𝐑𝐞𝐚𝐥 Obf Botz ]\x1b[0m`;
...
async function sendJoinButtons(ctx) {
    const buttons = [
        [
            { text: "📢 Channel 1", url: "https://t.me/aboutyamzz" },
        ],
        [
            { text: "📢 Channel 2", url: "https://t.me/aboutyamzz2" },
        ],
        [
            { text: "📢 Channel 3", url: "https://t.me/encyamzz" },
        ],
        [
    ...
    return ctx.reply(
        "⚠️ Kamu harus join semua channel berikut dulu untuk menggunakan bot!",
        {
...
bot.start(async (ctx) => {
    ...
    const inlineKeyboard = [
        [
            { text: '𝑪𝒉𝒂𝒏𝒏𝒆𝒍 「🔊」', url: 'https://t.me/aboutyamzz' },
            { text: '𝑫𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓 「👤」', url: 'https://t.me/yamzzzx' }
        ],
        [
            { text: '𝑰𝒏𝒇𝒐 𝑩𝒐𝒕 「🤖」', callback_data: 'infobot' },
            { text: '𝐒𝐂𝐑𝐈𝐏𝐓 𝐁𝐎𝐓 「📁」', callback_data: 'script' },
...
╚─═─═─═─═─═─═─═─═─═─═─═─⊱
    `, {
        ...
        reply_markup: {
            inline_keyboard: [
                [
                    { text: '𝑪𝒉𝒂𝒏𝒏𝒆𝒍 「🔊」', url: 'https://t.me/𝐘𝐚𝐦𝐳𝐳𝐑𝐞𝐚𝐥ch2' },
                    { text: '𝑫𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓 「👤」', url: 'https://t.me/yamzzzx' }
                ],
                [
                    { text: '𝑰𝒏𝒇𝒐 𝑩𝒐𝒕 「🤖」', callback_data: 'infobot' },               
...
users.add(ctx.from.id);
    ...
    if (!isMember) {
        return ctx.replyWithMarkdown(
            ...
            "[ᴊᴏɪɴ sᴇᴋᴀʀᴀɴɢ](https://t.me/aboutyamzz)"
        );
    }
    ...
    try {
        ...
        await ctx.replyWithDocument(
            ...
            { caption: "✅ *File terenkripsi siap!*\nSUKSES ENCRYPT BY 𝐘𝐚𝐦𝐳𝐳𝐑𝐞𝐚𝐥 🕊", parse_mode: "Markdown" }
        );
  ...
  const isMember = await checkChannelMembership(ctx);
    ...
    try {
        ...
        await ctx.replyWithMarkdown(
            ...
            `_Level: ${encryptionLevel} | Powered by 𝐘𝐚𝐦𝐳𝐳𝐑𝐞𝐚𝐥`
        );
        await ctx.replyWithDocument(
            { source: encryptedPath, filename: `eval-encrypted-${file.file_name}` },
            { caption: "✅ *File terenkripsi siap!*\n_SUKSES ENCRYPT BY 𝐘𝐚𝐦𝐳𝐳𝐑𝐞𝐚𝐥 🕊", parse_mode: "Markdown" }
        );
...
bot.on("callback_query", async (ctx) => {
    ...
    if (data === "check_join") {
        ...
        if (!isMember) {
            return ctx.answerCbQuery("⚠️ Kamu belum join semua channel!", { show_alert: true });
        }

        await ctx.answerCbQuery("✅ Terima kasih sudah join!");

        return ctx.editMessageText("✅ Terverifikasi!\nSekarang kamu bisa memakai bot ✅");
    }
});
...
// Jalankan bot
bot.launch(() => log("Encrypt Bot by 𝐘𝐚𝐦𝐳𝐳𝐑𝐞𝐚𝐥 berjalan..."));
...
bot.action("script", (ctx) => {
    ctx.editMessageCaption(`
┏═━═━⫹⫺ 𝐈𝐍𝐅𝐎 𝐒𝐂𝐑𝐈𝐏𝐓 𝐁𝐎𝐓 🔒
┃ 𝙼𝙰𝚄 𝚂𝙲𝚁𝙸𝙿𝚃 𝙽𝚈𝙰 ? 𝚂𝙸𝙻𝙰𝙷𝙺𝙰𝙽 𝙾𝚁𝙳𝙴𝚁 𝙳𝙸 𝙾𝚆𝙽𝙴𝚁 𝙺𝚄 𝙺𝙰𝙺 🙏🏻
║ ☍ ᴄʜᴀɴɴᴇʟ : @aboutyamzz
┃ ☍ ᴅᴇᴠᴇʟᴏᴘᴇʀ : @yamzzzx
╚━═━═━═━═━═━═━═━═━═━❍
Stay secure with Yamzz?! `
